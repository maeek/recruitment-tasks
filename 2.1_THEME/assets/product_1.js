jQuery( document ).ready(function( $ ) {
    'use strict';
    var p,
    productContainer = $('.shb-product-1'),
    PRODUCT_1 = {
        settings:{
        ...
        init: function(){
            p = this.settings;
            console.log('PRODUCT_1 INIT', p.productContainer);
            Shop.productHandler = p.productContainer;
        ...
        windowLoadHandler: function() {
            ...
            if ( p.isMobile === 1 ) {
                PRODUCT_1.mobileGallery();
            } else {
                PRODUCT_1.desktopGallery();
            }

            if ( p.variantsHandler.length ) {
                Shop.productHandler = p.productContainer.productVariants({
                    productSectionContainer: '.shb-product-1',
        ...
        mobileGallery: function() {
            ...
            initPhotoSwipeFromDOM('.shb-product-1 .shb-main-gallery img', p.mainGallery.data('zoom-ratio'));
        },

        desktopGallery: function() {
            var thumbItem = p.thumbsGallery.find('.shb-thumb-item'),
                maxThumbsToShow = PRODUCT_1.slidesToShow();
            p.mainGallery.on('init', function(){
                p.mainGallery.fadeTo("slow", 1);
                p.mainGallery.find('.shb-gallery-item').css("position", "relative");
            });
            ...
            p.thumbsGallery.imagesLoaded(function(){
                if (thumbItem.length > maxThumbsToShow) {
                    ...
                    PRODUCT_1.showThumbsGallery(thumbItem);
                }
